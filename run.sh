...
LM_HEAD_WEIGHTED_COLUMNWISE_GRADIENT_CENTERING_ARG="${LM_HEAD_WEIGHTED_COLUMNWISE_GRADIENT_CENTERING_ARG:-}"
...
export LM_HEAD_OPTIMIZER_PEAK_LR_ARG
...
build_train_args_for_seed() {
  ...
  if [[ -n "$LM_HEAD_GRADIENT_CENTERING_ARG" ]]; then
    case "${LM_HEAD_GRADIENT_CENTERING_ARG,,}" in
      off|pre|post)
        TRAIN_ARGS+=("opt.lm_head_gradient_centering=${LM_HEAD_GRADIENT_CENTERING_ARG,,}")
        ;;
      *)
        echo "ERROR: LM_HEAD_GRADIENT_CENTERING_ARG must be one of: off, pre, post, or unset. Got: $LM_HEAD_GRADIENT_CENTERING_ARG" >&2
        exit 1
        ;;
    esac
  fi

  if [[ -n "$LM_HEAD_WEIGHTED_COLUMNWISE_GRADIENT_CENTERING_ARG" ]]; then
    case "${LM_HEAD_WEIGHTED_COLUMNWISE_GRADIENT_CENTERING_ARG,,}" in
      off|pre|post)
        TRAIN_ARGS+=("opt.lm_head_weighted_columnwise_gradient_centering=${LM_HEAD_WEIGHTED_COLUMNWISE_GRADIENT_CENTERING_ARG,,}")
        ;;
      *)
        echo "ERROR: LM_HEAD_WEIGHTED_COLUMNWISE_GRADIENT_CENTERING_ARG must be one of: off, pre, post, or unset. Got: $LM_HEAD_WEIGHTED_COLUMNWISE_GRADIENT_CENTERING_ARG" >&2
        exit 1
        ;;
    esac
  fi
  ...
  if [[ -n "$USE_LM_HEAD_SGD_MOMENTUM" ]]; then
    if [[ "${USE_LM_HEAD_SGD_MOMENTUM,,}" == "true" ]]; then
      TRAIN_ARGS+=("opt.lm_head_optimizer.type=sgd_momentum")
    elif [[ "${USE_LM_HEAD_SGD_MOMENTUM,,}" == "false" ]]; then
  ...
  if [[ -n "$LM_HEAD_OPTIMIZER_PEAK_LR_ARG" ]]; then
    TRAIN_ARGS+=("opt.lm_head_optimizer.peak_lr=${LM_HEAD_OPTIMIZER_PEAK_LR_ARG}")
  fi
